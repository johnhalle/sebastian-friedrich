...
**Cloudinary ist ausschließlich für Bilder.** PDFs kommen ins GitHub-Repo (s. unten).
...
Jeder Eintrag: Kategorie-Badge · Zeitraum · Titel · Ort · Kurztext (gekürzt) · Titelbild.
Klick öffnet Detailansicht (gleiche Seite, History-API) mit vollständigem Fließtext,
optionaler Bildergalerie (Lightbox), optionalem PDF-Viewer, optionalem externem Link.

**PDF-Viewer:** Direkter `<iframe>` auf die Cloudinary-URL. Kein Drittanbieter-Viewer.
Anzeige nur wenn `pdf`-Feld befüllt — HEAD-Check entfällt bei Cloudinary-URLs.
...
4. **GitHub PAT anlegen** — fine-grained, nur Repository sebastian-friedrich,
   Contents Read & Write
5. **News-Bilder vervollständigen** — Titelbilder + Galeriefotos für beide Einträge
   auf Cloudinary `sebastian-friedrich/news/` — URLs bereits in data.json eingetragen.
   Katalogpräsentation 2026: Galerie noch leer (keine weiteren Fotos vorhanden).
